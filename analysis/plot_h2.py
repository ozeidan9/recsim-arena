"""Generate the H2 figure: quality gate shifts the creator equilibrium.

Compares the IPPO equilibrium trajectories under M1 (single-stage, H1 baseline)
and M3 (two-stage + quality gate). Produces a 2×2 panel figure
(diversity entropy, mean quality, mean bait, Gini) saved to paper/figures/.

  M1 logs: results/h1_seed<seed>/ippo_log.json
  M3 logs: results/h2_m3_seed<seed>/ippo_log.json

Usage:
    python analysis/plot_h2.py                        # seed=42
    python analysis/plot_h2.py --seed 0
    python analysis/plot_h2.py --seeds 42 0 7 13 99   # multi-seed CI bands
"""
...
def plot_h2(seeds: list[int], out_dir: Path = FIGURE_DIR) -> None:
    out_dir.mkdir(parents=True, exist_ok=True)

    m1_logs = [load_log("h1_seed{seed}", s) for s in seeds]
    m3_logs = [load_log("h2_m3_seed{seed}", s) for s in seeds]
    ...
    fig.suptitle(
        "H2: Quality gate shifts equilibrium toward quality (M3 vs M1)",
        fontsize=13,
        fontweight="bold",
    )
    ...
    series = [
        ...
        ("M3 (quality gate)", m3_logs, "#2ca02c", "-"),
    ]

    for ax, (key, title, ylabel) in zip(axes.flat, metrics):
        for label, logs, color, ls in series:
            xs, mean, std = _extract_series(logs, key)
            ax.plot(xs, mean, color=color, label=label, linewidth=2, linestyle=ls)
            if len(seeds) > 1:
    ...
    pdf_path = out_dir / f"h2_gate_seed{seed_str}.pdf"
    png_path = out_dir / f"h2_gate_seed{seed_str}.png"
